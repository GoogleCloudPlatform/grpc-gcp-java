...
public class GcpMultiEndpointChannelOtelMetricsTest {
  ...
  public void emitsOtelEndpointSwitchMetric() {
    ...
    // Collect metrics and verify endpoint_switch exists with switch_type attributes.
    Collection<MetricData> metrics = reader.collectAllMetrics();
    assertNotNull(metrics);
    List<MetricData> endpointSwitch =
        metrics.stream()
            .filter(m -> m.getName().equals("test/me/" + GcpMetricsConstants.METRIC_ENDPOINT_SWITCH))
            .collect(Collectors.toList());
    assertTrue(!endpointSwitch.isEmpty());
    MetricData m = endpointSwitch.get(0);
    // Ensure points exist for FALLBACK/RECOVER/REPLACE (values may be 0 initially)
    boolean hasFallback =
        m.getLongGaugeData().getPoints().stream()
            .anyMatch(p -> p.getAttributes().asMap().values().contains(GcpMetricsConstants.TYPE_FALLBACK));
    boolean hasRecover =
        m.getLongGaugeData().getPoints().stream()
            .anyMatch(p -> p.getAttributes().asMap().values().contains(GcpMetricsConstants.TYPE_RECOVER));
    boolean hasReplace =
        m.getLongGaugeData().getPoints().stream()
            .anyMatch(p -> p.getAttributes().asMap().values().contains(GcpMetricsConstants.TYPE_REPLACE));
